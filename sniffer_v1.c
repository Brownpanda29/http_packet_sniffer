...
void process_sniffed_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    if (ether_type == ETH_P_IP) {
        ...
        if (protocol == IPPROTO_TCP) {
            ...
            struct udphdr *udp_header = (struct udphdr *)(packet + total_header_length);
            uint16_t udp_header_length = sizeof(struct udphdr);

            const char *payload = (const char *)(packet + total_header_length + udp_header_length);
            // Process UDP payload
        }
